...
export default function AddCouponModal({ from, to, onClose, onCreated }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setDone(true);
    setBusy(false);
    // Wait for success animation to finish, THEN refresh + close
    // No jitter: onCreated fires silently in background, onClose after delay
    setTimeout(async () => {
      await onCreated();
      onClose();
    }, 1600);
  ...
  return (
    // No AnimatePresence here — the parent controls mounting/unmounting
    // which avoids the double-render jitter on exit
    <motion.div
      key="backdrop"
    ...
    >
      <motion.div
        ...
        /* Fixed min-height prevents the card from collapsing/jumping when
           the form switches to the success state */
        className="surface-raised w-full max-w-[420px] overflow-hidden rounded-2xl"
        style={{
          boxShadow: '0 48px 120px rgba(0,0,0,0.65), 0 0 0 1px rgba(232,213,176,0.07)',
          minHeight: 340,
        }}
      >
            ...
            <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className="px-6 pb-5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {EXAMPLES.map(ex => (
                    <motion.button key={ex.name} whileTap={{ scale: 0.96 }}
                      onClick={() => fill(ex)} className="pill pill-pink cursor-pointer transition-all">
                      {ex.emoji} {ex.name}
                    </motion.button>
                  ))}
                </div>
