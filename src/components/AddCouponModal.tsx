  ...
  return (
    ...
    >
      <motion.div
        ...
        transition={{ duration: 0.72, delay: 2.4, ease: [0.76, 0, 0.24, 1] as const }}
        style={{ position: 'absolute', inset: 0, background: 'linear-gradient(160deg, #0C0B0A 0%, #1a1510 50%, #0C0B0A 100%)', transformOrigin: 'bottom', zIndex: 1 }}
      />
      <motion.div
        initial={{ opacity: 0, scale: 0.4 }}
        animate={{ opacity: [0, 0.55, 0.35], scale: [0.4, 1.8, 1.4] }}
        transition={{ duration: 1.4, ease: 'easeOut' }}
        style={{ position: 'absolute', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle, rgba(232,213,176,0.12) 0%, rgba(196,154,108,0.06) 40%, transparent 70%)', zIndex: 2, pointerEvents: 'none' }}
      />
      <motion.div
        initial={{ scaleX: 0, opacity: 0 }}
        animate={{ scaleX: [0, 1, 1, 0], opacity: [0, 0.5, 0.5, 0] }}
        transition={{ duration: 1.6, times: [0, 0.25, 0.75, 1], ease: 'easeInOut' }}
        style={{ position: 'absolute', width: '60%', height: 1, background: 'linear-gradient(90deg, transparent, rgba(232,213,176,0.6), transparent)', zIndex: 3 }}
      />
      <div style={{ position: 'relative', zIndex: 4, textAlign: 'center', padding: '0 24px' }}>
          ...
          {letters.map((ch, i) => (
            <motion.span key={i}
              ...
              style={{
                ...
                color: 'rgba(255,250,242,0.90)', letterSpacing: '-0.02em',
        ...
        <motion.div
          ...
          style={{ width: 64, height: 1, margin: '0 auto 18px', background: 'linear-gradient(90deg, transparent, rgba(232,213,176,0.50), transparent)', originX: 0.5 }}
        />
        <motion.p
          initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 1.15, duration: 0.55 }}
          style={{ fontSize: 14, color: 'rgba(255,250,242,0.36)', fontWeight: 300, fontFamily: "'Cormorant Garamond', Georgia, serif", fontStyle: 'italic' }}
        >{sub}</motion.p>
        <motion.p
...
export default function AddCouponModal({ from, to, onClose, onCreated }: Props) {
  ...
  return (
    <>
      ...
      >
        {/* Single style prop — merges motion values + static styles */}
        <motion.div
          ...
          style={{
            rotateX, rotateY,
            perspective: 900,
            boxShadow: '0 0 0 1px rgba(232,213,176,0.10), 0 32px 80px rgba(0,0,0,0.75)',
            minHeight: 340,
          }}
          onMouseMove={e => {
